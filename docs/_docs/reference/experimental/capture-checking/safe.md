...
Note that the restrictions imposed by safe mode do not rule out throwing exceptions. This is the case even though exceptions are a form of side effect that can leak information. The reason for this gap in coverage is that exceptions are so common that it's impractical to control them in a static type system. For instance, exceptions can be thrown on buffer overflows, stack overflows, divisions by zero, or out of memory events. Rather than trying to track all these conditions in capabilities, we rely on runtime protection. Specifically, the calls to agent-generated functions are supposed to return results of type `Try`. `Try` is a standard library type in Scala that reifies any non-fatal exception thrown by the agent as a failure value. The failure value can be wrapped in a `Classified` if needed, which would prevent leakage through exceptions in our scenario. As an added
safety measure, any leakage of other capabilities as fields of exceptions is also prevented since exception classes are treated as pure.
