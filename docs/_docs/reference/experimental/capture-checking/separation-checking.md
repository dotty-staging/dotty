...
Conceptually, it builds on the model of [scoped capabilities](scoped-capabilities.md) by adding non-aliasing guarantees for exclusive capabilities.
These rules apply only to exclusive capabilities and their read-only versions. Capabilities whose type extends `SharedCapability` are exempt.
