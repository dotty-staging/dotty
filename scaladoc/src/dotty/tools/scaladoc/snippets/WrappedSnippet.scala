...
object WrappedSnippet:
  ...
  case class WrappedLineMapping(sourceLine: Int, relativeLine: Int, wrappedColumnOffset: Int):
    def sourcePosition(
      ...
      val lineOffset = sourceFile match
        ...
        case sf: SourceFile => Try(sf.lineToOffset(sourceLine)).toOption
  ...
  def apply(
    ...
    globalImports.foreach { case (line, sourceLine, relativeLine) =>
      printlnWithMapping(
        line,
    ...
    rest.foreach { case (line, sourceLine, relativeLine) =>
      printlnWithMapping(
        (" " * indent) + line,
