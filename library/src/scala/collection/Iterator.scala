...
trait Iterator[+A] extends IterableOnce[A] with IterableOnceOps[A, Iterator, Iterator[A]] { self =>
  ...
  def duplicate: (Iterator[A], Iterator[A]) = {
    ...
    @annotation.nullTrackable var ahead: Iterator[A] | Null = null
...
object Iterator extends IterableFactory[Iterator] {
    ...
    def hasNext =
      if (currentHasNextChecked) true
        ...
        @tailrec def merge(): Unit =
          if (current.nn.isInstanceOf[ConcatIterator[_]]) {
            val c = current.nn.asInstanceOf[ConcatIterator[A]]
            current = c.current
  ...
  private final class UnfoldIterator[A, S](init: S)(f: S => Option[(A, S)]) extends AbstractIterator[A] {
    ...
    override def next(): A = {
      if (hasNext) {
        val (value, newState) = nextResult.nn.get  // 使用.nn安全访问
        state = newState
        nextResult = null
        value
      } else Iterator.empty.next()
