...
trait AsScalaConverters {
   ...
   */
  def asScala[A](i: ju.Iterator[A]): Iterator[A] | Null = i match {
    case null                            => null
   ...
   * @return  A Scala `Iterator` view of the argument.
   */
  def asScala[A](e: ju.Enumeration[A]): Iterator[A] | Null = e match {
    case null                            => null
    case wrapper: IteratorWrapper[A @uc] => wrapper.underlying
   ...
   */
  def asScala[A](i: jl.Iterable[A]): Iterable[A] | Null = i match {
    case null                            => null
   ...
   * @return  A Scala `Iterable` view of the argument.
   */
  def asScala[A](c: ju.Collection[A]): Iterable[A] | Null = c match {
    case null                            => null
    case wrapper: IterableWrapper[A @uc] => wrapper.underlying
   ...
   */
  def asScala[A](l: ju.List[A]): mutable.Buffer[A] | Null = l match {
    case null                                 => null
   ...
   */
  def asScala[A](s: ju.Set[A]): mutable.Set[A] | Null = s match {
    case null                              => null
   ...
   */
  def asScala[K, V](m: ju.Map[K, V]): mutable.Map[K, V] | Null = m match {
    case null                                     => null
   ...
   */
  def asScala[K, V](m: juc.ConcurrentMap[K, V]): concurrent.Map[K, V] | Null = m match {
    case null                                        => null
   ...
   * @return  A Scala mutable `Map` view of the argument.
   */
  def asScala[K, V](d: ju.Dictionary[K, V]): mutable.Map[K, V] | Null = d match {
    case null                                     => null
   ...
   */
  def asScala(p: ju.Properties): mutable.Map[String, String] | Null = p match {
    case null => null
