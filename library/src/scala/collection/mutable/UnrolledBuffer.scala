...
sealed class UnrolledBuffer[T](implicit val tag: ClassTag[T])
  ...
  def iterator: Iterator[T] = new AbstractIterator[T] {
    ...
    private def scan(): Unit = {
      pos += 1
      while (node != null && pos >= node.nn.size) {
        pos = 0
        node = node.nn.next
  ...
  class Unrolled[T: ClassTag] private[collection] (var size: Int, var array: Array[T], var next: Unrolled[T] | Null, val buff: UnrolledBuffer[T] | Null) {
    ...
    @tailrec final def insertAll(idx: Int, t: scala.collection.IterableOnce[T], buffer: UnrolledBuffer[T]): Int = {
      if (idx < size) {
        ...
        // update new next
        val newnextnode = new Unrolled[T](0, new Array(array.length), null.asInstanceOf[Unrolled[T] | Null], buff)
        ...
        nullout(idx, size)
        size = idx
        next = null.asInstanceOf[Unrolled[T] | Null]
