...
class RangeConsistencyTest {
  ...
  def test_SI9388(): Unit =  {
    ...
    assert(r.sum(using possiblyNotDefaultNumeric) == Int.MinValue)
    assert(nr.sum(using possiblyNotDefaultNumeric) == Int.MinValue)
  }
  ...
  def test_SI10086(): Unit =  {
    ...
    assert(nr.min(using customIntegral.reverse) == 9)
    assert(nr.max(using customIntegral.reverse) == 1)
    ...
    // Also test with custom ordering.
    assert(r.min(using aIsIntegral.reverse) == A(9))
    assert(r.max(using aIsIntegral.reverse) == A(1))
  }
}
