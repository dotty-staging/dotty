...
if [[ "${1:-}" != "--no-build" ]]; then
  echo "==> Building compiler JS and packing classpath..."
  sbt 'project scala3-compiler-sjs' 'compile; fastLinkJS; packClasspath'
fi

# --- Verify artifacts exist ---
for f in "$FASTOPT_DIR/main.js" "$CLASSPATH_BIN" "$INDEX_HTML"; do
  if [[ ! -f "$f" ]]; then
