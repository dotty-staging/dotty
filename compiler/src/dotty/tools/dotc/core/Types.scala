...
// --- NamedTypes ------------------------------------------------------------------
  ...
  abstract case class CapturingType(parent: Type, refs: CaptureSet) extends AnnotOrCaptType:
    ...
    def derivedCapturingType(parent: Type, capt: Type)(using Context): Type =
      parent.capturing(capt.captureSet)
  ...
  extends VariantTraversal with (Type => Type) { thisMap =>
    ...
    protected def derivedCapturingType(tp: CapturingType, parent: Type, refs: CaptureSet): Type =
