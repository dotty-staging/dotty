...
class Driver {
  ...
  protected def doCompile(files: List[AbstractFile])(using ictx: Context): Reporter =
    ...
    if !isOutline then inContext(ictx):
      ...
      def secondPassCtx(id: Int, group: List[AbstractFile]): Context =
        require(id >= 0, s"expected non-negative worker id, found: $id")
        val baseCtx = initCtx.fresh
          ...
          .setSetting(ictx.settings.YbatchWorkerId, id)
          .setSetting(ictx.settings.YoutlineClasspath, outlineOutput)
          .setCallbacks(ictx.store)
          .setReporter(ictx.reporter)
      ...
      if !firstPassCtx.reporter.hasErrors && scalaFiles.nonEmpty then
        def compileEager(
          ...
          val secondCtx = secondPassCtx(id, fileGroup)
        ...
        compileEager(0, scalaFiles)
        firstPassCtx.reporter
      else
        // TODO: extend Compiler for the second pass - Then we can sort out Java vs Scala more easily
        ...
        val optState = inContext(secondPassCtx(0, Nil)):
          val async = AsyncTastyHolder.init
          async.signalDepsComplete(done = true)
          async.sync()
        for bufferedReporter <- optState do
          bufferedReporter.syncReports()(using firstPassCtx)
        end for
