...
package object semanticdb {
  ...
  private val symbolBuilder = SemanticSymbolBuilder()
  /** Gets the SemanticDB name of the given symbol. */
  def symbolName(sym: Symbol)(using Context): String = {
    import dotty.tools.dotc.semanticdb.Scala3.given
    if sym.exists && !sym.isGlobal then
      throw IllegalArgumentException("symbolName is not valid for local symbols")
    symbolBuilder.symbolName(sym)
  }
}