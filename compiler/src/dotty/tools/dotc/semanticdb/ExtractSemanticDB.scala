...
class ExtractSemanticDB extends Phase:
  ...
  class Extractor extends TreeTraverser:
    ...
    def extract(tree: Tree)(using Context): Unit =
      traverse(tree)
      val fakeSyms = converter.fakeSymbols.map(_.symbolInfo(Set.empty, List.empty)(using LinkMode.SymlinkChildren, converter))
      ...
      tree match
          ...
          if !excludeDef(tree.pid.symbol) && tree.pid.span.hasLength then
            tree.pid match
              case tree: Select =>
                ...
                registerDefinition(tree.symbol, selectSpan(tree), Set.empty, tree.source, List.empty)
              case tree => registerDefinition(tree.symbol, tree.span, Set.empty, tree.source, List.empty)
        case tree: NamedDefTree =>
          if !tree.symbol.isAllOf(ModuleValCreationFlags) then
            ...
            if !excludeDef(tree.symbol) && tree.span.hasLength then
              registerDefinition(tree.symbol, tree.nameSpan, symbolKinds(tree), tree.source, tree.mods.annotations)
              ...
              registerSymbol(tree.symbol, symbolKinds(tree), tree.mods.annotations)
        ...
        case tree: Template =>
          ...
          if !excludeDef(ctorSym) then
            ...
            registerDefinition(ctorSym, tree.constr.nameSpan.startPos, Set.empty, tree.source, tree.mods.annotations)
        case tree: Apply =>
    ...
    private def registerSymbol(sym: Symbol, symkinds: Set[SymbolKind], annotations: List[untpd.Tree])(using Context): Unit =
      val sname = sym.symbolName
      val isLocal = sname.isLocal
      if !isLocal || !localNames.contains(sname) then
        if isLocal then
          localNames += sname
        symbolInfos += sym.symbolInfo(symkinds, annotations)(using LinkMode.SymlinkChildren, converter)

    private def registerSymbolSimple(sym: Symbol)(using Context): Unit =
      registerSymbol(sym, Set.empty, List.empty)
    ...
    private def registerDefinition(sym: Symbol, span: Span, symkinds: Set[SymbolKind], treeSource: SourceFile, annotations: List[untpd.Tree])(using Context) =
      val sname = sym.symbolName
      ...
      if !sym.is(Package) then
        registerSymbol(sym, symkinds, annotations)
    ...
    private def annotations(tree: NamedDefTree)(using LinkMode, TypeOps, SemanticSymbolBuilder, Context): List[Annotation] =
      if tree.symbol.isSelfSym then
        List.empty
      else
        tree.mods.annotations.map{
          annot =>
            Annotation(annot.typeOpt.toSemanticType(annot.symbol))
        }
    ...
    private def ctorParams(
      ...
      do
        ...
        if !excludeSymbol(vparam.symbol) then
          ...
          registerSymbol(vparam.symbol, symkinds, vparam.mods.annotations)
        traverse(vparam.tpt)
