...
abstract class Recheck extends Phase, IdentityDenotTransformer:
  ...
  class Rechecker(ictx: Context):
        ...
        case fntpe: MethodType =>
          ...
          def recheckArgs(args: List[Tree], formals: List[Type], prefs: List[ParamRef]): List[Type] = args match
            case arg :: args1 =>
              ...
              val formals1 =
                if fntpe.isParamDependent then formals.tail.map(_.substParam(prefs.head, argType))
                else formals.tail
    ...
    def recheckBlock(bindings: List[Tree], expr: Tree, pt: Type)(using Context): Type =
      recheckStats(bindings)
      val exprType = recheck(expr, pt.dropIfProto)
      TypeOps.avoid(exprType, localSyms(bindings).filterConserve(_.isTerm))
    ...
    def checkConforms(tpe: Type, pt: Type, tree: Tree)(using Context): Unit = tree match
      ...
      case _ =>
        ...
        if !isCompatible then
          pt match
            case WildcardType(TypeBounds(_, hi)) =>
              println(hi.typeSymbol)
            case _ =>
